...
def unpack_bar3ptfn(filelist_iter, loc="", transition_form=False, simplify=False):
    ...
    for filename in filelist_iter:
        ...
        if simplify:
            ...
            print("".join([FormatKappa(y) for y in forward_κ]))
            κ_string = [
                    ...
                    if (
                        type(
                            data[latt_size_str][ferm_act_string][κ_str][
                                source_sink_string
                            ][seq][p_str][form_fac_str]
                        )
                        == collections.defaultdict
                        and len(
                            data[latt_size_str][ferm_act_string][κ_str][
                                source_sink_string
                            ][seq][p_str][form_fac_str].keys()
                        )
                        == 0
                    ):
                        data[latt_size_str][ferm_act_string][κ_str][source_sink_string][
                            seq
                        ][p_str][form_fac_str] = [record_sliced]
                    else:
                        data[latt_size_str][ferm_act_string][κ_str][source_sink_string][
                            seq
                        ][p_str][form_fac_str].append(record_sliced)
                ...
                for source_sink, lvl4 in lvl3.items():
                    for seq, lvl5 in lvl4.items():
                        for p, lvl6 in lvl5.items():
                            out_dir = (
                                loc
                                + f"/barspec/{latt_size}/{ferm_act}/{κ}/{seq}"
                                + f"/{source_sink}/{p}/"
                            )

                            os.system(f"mkdir -p {out_dir}")

                            for form_fac, lvl6 in lvl5.items():
                                ncfg = len(lvl6)
...
def FormatMom(mom):
    return "p" + "".join([f"{p_i:+d}" for p_i in p])
...
def FormatFormFac(form_fac_number, deriv):
    gamma_num = form_fac_number % (4 ** deriv)
