...
def unpack_bar3ptfn(filelist_iter, loc="", transition_form=False, simplify=False):
    ...
    for filename in filelist_iter:
        ...
        current_κ_out = [
            float(x.find("SequentialProp_record_info")
            .find("SequentialProp")
            .find("SeqProp")
            .find("FermionAction")
            .find("Kappa")
        ...
        if simplify:
            κ_simplified = set(forward_κ+[current_κ_in]+[current_κ_out])
            κ_string = sum([FormatKappa(k) for k in κ_simplified])
