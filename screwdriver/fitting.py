...
def χ2(func, params, xdata, ydata):
    # Rows are variables, columns bootstraps
    cov_mat = np.cov(ydata)
    cov_mat_inv = np.linalg.inv(cov_mat)
    diff = func(xdata, params) - np.mean(ydata, axis=1)
...
def chiral_extrapolation_dict(mass_array, data, fit_func):
    ...
    for p in range(len(data[list(data.keys())[0]][0, :])):
        ...
        x_linspace = np.linspace(min(np.min(mass_array), 0), max(np.max(mass_array), 0))
